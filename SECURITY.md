...
- **Filesystem scope still bounds writes to a small set of host paths.** Even though `~/.claude` is mounted read-write (see *Open issues* below for the trade-off), the in-container AI still cannot reach the rest of `~`, your other repos, or `~/.ssh`/`~/.aws`/etc. Native Claude with auto-mode has no such bound.
...
- **`~/.claude` is read/write inside the container.** The in-container AI can install/uninstall plugins, modify `settings.json` (including `enabledPlugins`, `hooks`, `permissions`), and write to `plugins/cache/` — and any of those changes propagate to subsequent **host** `claude` sessions, which would then execute them. This was previously addressed by stacking read-only mounts on `settings.json` and `plugins/`, but those mounts broke normal Claude operation: plugin install requires writing `enabledPlugins` to `settings.json`, marketplace refresh requires writing under `plugins/marketplaces/`, etc. The current trade-off is to trust the in-container AI not to backdoor host claude state. If you want stricter isolation, layer your own RO bind-mounts on `~/.claude/plugins/cache/` and `~/.claude/settings.json` and accept that plugins won't be manageable from inside the container.
