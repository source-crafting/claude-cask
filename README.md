...
| `~/.claude`                   | `/home/claude-cask/.claude` (plus a symlink at the host home path — see below)                                | Claude Code config dir: settings, sessions, plugins. Read-write, with read-only overlays on `settings.json` and `plugins/cache/` (the persisted exec surface — installed skill/hook/script files). Marketplace metadata, install counts, and `installed_plugins.json` stay writable so plugin loading and refreshes work. The entrypoint also creates a symlink so the host home path (e.g. `/Users/<you>`) resolves to `/home/claude-cask` inside the container, which makes the absolute paths recorded in `plugins/installed_plugins.json` and `plugins/known_marketplaces.json` resolve correctly so installed plugins (and their skills) load. |
